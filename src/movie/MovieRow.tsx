...
export default function MovieRow({ title, movies }: MovieRowProps) {
    ...
    return (
        <div className="mb-8">
            <h2 className="text-4xl font-bold text-white mb-6 px-4 sm:px-6 lg:px-8 border-l-4 border-red-600 ml-4 sm:ml-6 lg:ml-8">
                {title}
            </h2>
            <div className="relative group overflow-hidden">
                ...
                >
                    ...
                    {movies.map((movie, index) => (
                        <div key={`${movie.id}-${index}`} className="w-[300px] md:w-[350px] flex-none">
                            <MovieCard movie={movie} />
                        </div>
                    ))}
                    {/* Duplicated Set for Infinite Scroll */}
                    {movies.map((movie, index) => (
                        <div key={`${movie.id}-${index}-duplicate`} className="w-[300px] md:w-[350px] flex-none">
                            <MovieCard movie={movie} />
                        </div>
                    ))}
                </div>
